...
# Groq AI 智能体推理模块
# =====================================================================
GROQ_API_KEY = "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"

def call_groq_agent(result: dict, company_name: str, industry: str) -> str:
    """
    将规则引擎识别出的风险数据发给 Groq LLM，
    让真正的 AI 进行审计专家级推理，输出完整中文分析报告。
    """
    import requests
    ...
    # 构造发给 AI 的上下文
    risk_summary = "\n".join([
        f"- 【{r['风险等级']}风险·{r['最终评分']}分】{r['风险类型']}：{r['触发规则']}（{r.get('业务合理性判断','')}）"
        for r in risks
    ]) or "- 未发现显著异常"

    prompt = f"""你是一名具有15年经验的注册会计师，正在对"{company_name}"（{industry}行业）进行年度财务报表审计风险评估。

以下是AI规则引擎与XGBoost模型已识别出的量化风险信号：

【核心财务指标】
- 营收增长率：{m.get('营收增长率', 0):.1f}%
- 毛利率：{m.get('毛利率', 0):.1f}%
- 净利率：{m.get('净利率', 0):.1f}%
- 应收账款周转率：{m.get('应收账款周转率', 0):.1f}次
- 存货周转率：{m.get('存货周转率', 0):.1f}次
- 资产负债率：{m.get('资产负债率', 0):.1f}%
- 经营现金流/净利润：{m.get('经营现金流_净利润比', 0):.2f}
- 应收账款增长率：{m.get('应收账款增长率', 0):.1f}%

【已识别风险项】（总体风险等级：{overall}）
{risk_summary}

请你作为审计专家，完成以下任务：
1. 用专业的审计语言，综合分析上述风险信号之间的内在联系，判断是否存在财务造假或重大错报的可能性
2. 指出最需要关注的核心风险点及其审计逻辑
3. 给出3-5条具体、可操作的审计程序建议，要有针对性，不要泛泛而谈
4. 对审计风险整体作出专业定性结论

要求：语言专业、逻辑严密、300-500字、直接给出分析内容（不需要说"好的"或重复题目）。"""
    ...
    try:
        ...
        if "choices" in data and data["choices"]:
            ...
            return f"AI推理失败：{data.get('error', {}).get('message', '未知错误')}"
    except Exception as e:
        return f"连接 Groq 失败：{e}"


# =====================================================================
# PDF 导出（修复中文乱码）
# =====================================================================
def generate_pdf(result, company_name, industry="制造业", news_data=None, groq_analysis=None):
    try:
        ...
        if groq_analysis:
            ...
            story.append(Paragraph("三、AI 智能体推理结论（Groq · LLaMA-3.3-70B）",
                                   T("sec_ai", fontSize=13, textColor=colors.HexColor("#1A1A2E"), spaceBefore=10, spaceAfter=6)))
            story.append(Paragraph(
                "以下内容由大语言模型基于规则引擎量化数据进行审计专家级语义推理生成，需注册会计师复核确认：",
                T("ai_note", fontSize=9, textColor=colors.HexColor("#888"), spaceAfter=6)
            ))
            # 按段落拆分，避免单个超长段落
            for para in groq_analysis.split("\n"):
                para = para.strip()
                if para:
                    story.append(Paragraph(para, T(f"ai_{id(para)}", fontSize=9,
                                                   textColor=colors.HexColor("#333"), spaceBefore=3, spaceAfter=3, leading=15)))

        # 舆情数据（如有）
        if news_data:
            story += [Spacer(1, 0.5*cm)]
            story.append(Paragraph("四、舆情监控信号", T("sec3", fontSize=13, textColor=colors.HexColor("#1A1A2E"), spaceBefore=10, spaceAfter=6)))
            for n in news_data:
...
for k, v in [("done", False), ("result", None), ("inc", None), ("bal", None), ("cf", None),
             ...
             ("news_data", None), ("policy_data", None), ("groq_analysis", None)]:
...
with st.sidebar:
    ...
    if st.button("🔄 重置", use_container_width=True):
        for k in ["done", "result", "inc", "bal", "cf", "pdf", "company", "years_list", "news_data", "policy_data", "groq_analysis"]:
...
def normalize_ticker(ticker: str) -> str:
    """
    将用户输入的股票代码标准化为 yfinance 可识别格式：
    - 6位纯数字 → 自动判断沪深并补后缀
      60xxxx / 68xxxx → .SS（上交所）
      其余          → .SZ（深交所）
    - 其他格式（AAPL、0700.HK 等）原样返回
    """
    t = ticker.strip()
...
with tab_yf:
    ...
    st.markdown("支持全球股票：**A股直接输入6位数字**（000001、600519）、美股（AAPL）、港股（0700.HK）等")
    yc1, yc2, yc3 = st.columns([2, 1, 1])
    with yc1:
        yf_ticker = st.text_input("股票代码", placeholder="例：000001、600519、AAPL、0700.HK",
    ...
    if yf_btn:
        if not yf_ticker or not yf_ticker.strip():
            ...
            resolved_ticker = normalize_ticker(yf_ticker.strip())
            with st.spinner(f"正在从 Yahoo Finance 拉取 {resolved_ticker} 的财务数据..."):
                try:
                    inc, bal, cf, cname, yrs = fetch_yahoo_finance(resolved_ticker, yf_years)
                    ...
                    st.session_state.news_data = fetch_news_sentiment(cname, resolved_ticker)
                    st.session_state.policy_data = fetch_policy_updates()
                    st.success(f"✅ 成功拉取：{cname}（{resolved_ticker}）· {len(yrs)} 年数据")
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ 拉取失败：{e}")
...
with tab_live:
    ...
    with c1:
        ticker_input = st.text_input("股票代码", placeholder="例：000001、600519、002594（支持沪深A股6位代码）",
    ...
    if fetch_btn:
        ...
        elif not ticker_input or not ticker_input.strip():
            st.error("⚠️ 请输入股票代码")
        else:
...
if not st.session_state.done:
    ...
    # ── Step 4：AI智能体深度分析 + 审计建议 + 导出 ──
    st.markdown('<div class="step-title"><span class="step-badge">4</span>AI 智能体深度分析与导出</div>', unsafe_allow_html=True)

    # ── Groq AI 智能推理 ──
    st.markdown("#### 🤖 AI 审计专家推理（Groq · LLaMA 3.3 70B）")
    st.markdown('<div style="background:#F0F7FF;border:1px solid #0066CC;border-radius:8px;padding:10px 16px;font-size:12px;color:#0066CC;margin-bottom:12px">'
                '本模块调用 Groq 云端大语言模型（LLaMA 3.3 70B），将规则引擎识别出的量化风险数据输入给 AI，'
                '由 AI 进行审计专家级语义推理，输出超越模板填空的真实智能分析。'
                '</div>', unsafe_allow_html=True)

    ai_col1, ai_col2 = st.columns([1, 3])
    with ai_col1:
        ai_btn = st.button("🧠 启动 AI 推理", use_container_width=True, type="primary",
                           disabled=(st.session_state.groq_analysis is not None))
    with ai_col2:
        if st.session_state.groq_analysis:
            st.success("✅ AI 推理完成")
        else:
            st.markdown("点击「启动 AI 推理」让 LLM 对风险数据进行专家级语义分析（约需 5-10 秒）")

    if ai_btn and st.session_state.groq_analysis is None:
        with st.spinner("🤖 AI 审计专家正在分析风险数据，进行多步推理..."):
            analysis = call_groq_agent(res, st.session_state.company, st.session_state.industry)
            st.session_state.groq_analysis = analysis
            st.rerun()

    if st.session_state.groq_analysis:
        st.markdown(
            f'<div style="background:#FFFFFF;border:1px solid #E8ECEF;border-radius:10px;'
            f'padding:20px 24px;margin:8px 0;line-height:1.8;font-size:14px;color:#1A1A2E;'
            f'border-left:4px solid #0066CC;box-shadow:0 2px 8px rgba(0,102,204,0.08)">'
            f'<div style="font-size:11px;color:#888;margin-bottom:10px;font-family:monospace">'
            f'🤖 Groq · LLaMA-3.3-70B · {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} · '
            f'审计对象：{st.session_state.company} · 行业：{st.session_state.industry}</div>'
            + st.session_state.groq_analysis.replace("\n", "<br>")
            + f'<div style="margin-top:12px;font-size:11px;color:#999;border-top:1px solid #EEE;padding-top:8px">'
            f'⚠️ 以上为 AI 推理结论，需注册会计师复核确认后方可采用</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        ...
        '<div class="advice-title">📌 关键审计发现（规则引擎输出 · 详细版请导出 PDF）</div>'
    ...
    with e1:
        if st.button("📄 生成 PDF 报告", use_container_width=True, type="primary"):
            with st.spinner("正在生成审计重点提示函（含完整证据链与AI分析）..."):
                ...
                if pdf:
                    ...
                    st.success("✅ PDF 生成成功（含 AI 推理结论、详细分析与证据链）")
                else:
                    st.error("PDF 生成失败")
    with e2:
